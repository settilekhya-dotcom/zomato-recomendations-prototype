"""
Database Setup module for Phase 1 - Simplified Version (No Pandas)
Creates and manages the SQLite database for storing restaurant data.
"""
...
class DatabaseManager:
    """
    Manages the SQLite database for storing restaurant data.
    ...
    def create_table(self):
        """
        Create the restaurants table with appropriate schema.
        """
    ...
    def insert_data(self, data: List[Dict[str, Any]], if_exists: str = 'replace'):
        """
        Insert data from list of dictionaries into the database.
        
        Args:
            data: List of dictionaries containing restaurant data
        ...
        # Filter columns to only include those that exist in our schema
        allowed_columns = [
        ...
        # Get intersection of data keys and allowed columns from the first record
    ...
    def get_cities(self) -> List[str]:
        """
        Get list of unique cities in the database.
    ...
    def get_cuisines(self) -> List[str]:
        """
        Get list of unique cuisines in the database.
    ...
    def query_by_city(self, city: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of dictionaries with restaurants in the specified city
        """
        if not self.connection:
            self.connect()
    ...
    def query_by_city_and_price(self, city: str, price_category: str) -> List[Dict[str, Any]]:
        """
        Query restaurants by city and price category.
        
        Args:
            city: City name
            price_category: Price category (budget, mid-range, premium)
            
        Returns:
            List of dictionaries with matching restaurants
        """
        if not self.connection:
            self.connect()
